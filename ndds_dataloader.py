...
class MultipleVertexJson(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        scale = 4
        if min (img.size) != min (img_size):
            # print (scale)
            scale = min (img.size) /( min (img_size) / 4.0)
        ...
        w_crop = np.random.randint(0, img_h - img_size[0]+1)
        h_crop = np.random.randint(0, img_w - img_size[1]+1)
        transform = transforms.Compose([transforms.Resize(min(img_size))])
        totensor = transforms.Compose([transforms.ToTensor()])
        ...
        # Updated scaling factor over the following images from 8->4 to get 64x64 maps
        w_crop = int(w_crop/4)
        h_crop = int(h_crop/4)
